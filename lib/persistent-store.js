...
function normalizePage(page) {
  ...
  if (page.template === 'ecommerce' && !blocks.some((b) => b.type === 'price')) {
    const orderIndex = blocks.findIndex((b) => b.type === 'order');
    const price = { id: Math.random().toString(36).slice(2,10), type:'price', currency:'৳', regular:1200, offer:899, showDiscount:true };
    const next = [...blocks];
    next.splice(orderIndex >= 0 ? orderIndex : next.length, 0, price);
...
export async function savePage(id, data) {
  const page = normalizePage({ ...data, id, updatedAt: new Date().toISOString() });
...
export async function getPage(id) { return normalizePage(await redis.get(`adpage:${id}`)); }
export async function listPages(ownerId) {
  ...
  return pages.filter(Boolean).map(normalizePage).sort((a,b) => new Date(b.updatedAt||0)-new Date(a.updatedAt||0));
}
