...
export async function POST(request) {
  try {
    ...
    // First 3 landing pages are free. Editing an existing page is always allowed.
    if (isNewPage && !active && pages.length >= FREE_PAGE_LIMIT) {
      return NextResponse.json({
        error: `You have used all ${FREE_PAGE_LIMIT} free landing pages. Please choose a subscription plan to create more pages.`,
    ...
    return NextResponse.json({
      ...
      quota: {
        freeLimit: FREE_PAGE_LIMIT,
        used: updatedPages.length,
        remaining: Math.max(0, FREE_PAGE_LIMIT - updatedPages.length),
        unlimited: active
      }
    });
  } catch (error) {
